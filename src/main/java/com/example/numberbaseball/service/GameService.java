...
public class GameService {
	...
	public void createOrJoinRoom(String userName, String roomCode, String sessionId) {
		...
		if (roomStatus.equals(RoomStatus.WAITING)) {
			...
			User counsterUser = room.getUserList()
				.stream()
			...
			UserResponseDTO userResponseDTO = new UserResponseDTO(sessionId, userName, roomCode,
				roomStatus == RoomStatus.READY);
			UserResponseDTO counterUserResponseDTO = new UserResponseDTO(counterSessionId, counsterUser.getUserName(),
				roomCode, roomStatus == RoomStatus.READY);
	...
	public void setNumber(String sessionId, NumberResponseDTO numberResponseDTO) {
		...
		try {
			...
			messageService.sendYourSecretNumber(sessionId, number.getNumber(), roomCode);

		} catch (Exception e) {
			messageService.sendErrorMessage(sessionId, e);
			messageService.sendRestartGame(sessionId, userName, roomCode);
		}
	}
	...
	public void numberGuess(String sessionId, NumberGuessDTO numberGuessDTO) {
		...
		User counterUser = room.getUserList()
			.stream()
			.filter(a -> !a.equals(user))
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException("User 없음"));
